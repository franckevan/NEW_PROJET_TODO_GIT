...
def modifier_statut_todo():
    ...
    try:
        ...
        if todos[choix]["statut"] == "À faire":
            ...
            todos[choix]["statut"] = "À faire"  # Erreur volontaire sans le 'e'
        else:
...
def supprimer_todo():
    lister_todos()
    try:
        choix = int(input("Sélectionnez le numéro du todo à supprimer : ")) - 1
        if choix < 0 or choix >= len(todos):
            print("Numéro invalide.")
            return
        
        confirmation = input(f"Êtes-vous sûr de vouloir supprimer '{todos[choix]['titre']}' ? (oui/non) : ")
        if confirmation.lower() == "oui":
            supprime = todos.pop(choix)
            print(f'Todo "{supprime["titre"]}" supprimé.')
        else:
